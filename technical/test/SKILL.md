...
1. **Target project**: Ask the user for the project path or git URL (default: current working directory).
   ...
   - If action is `implement`: offer a git worktree in `/tmp/""-skills/<project-slug>-tests`, create a `test/<description>` branch
...
1. Read `.""-skills.yaml` for test config.
2. **Explicitly prompt for test coverage target** if not configured:
   ...
   This value is stored in `.""-skills.yaml` under `tests.coverage_target`.
...
3. **Explicitly prompt for test strategy** if not configured:
   ...
   This value is stored in `.""-skills.yaml` under `tests.strategy`.
...
| Category                | What it tests                                           | Examples                                                                                                                                                   |
| ----------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Core user flows**     | End-to-end journeys work correctly                      | Registration flow pro""ces verified account, help request creation notifies nearby volunteers, mission lifecycle transitions are valid                     |
...
| **Accessibility behavior** | A11y features work correctly                             | Screen reader announces correct label, focus moves to modal on open, re""ced motion preference respected                                 |
...
Pro""ce a **coverage matrix** that maps rules to tests, not lines to tests.
...
**Impact score**: X/10 (weighted: T1 _ 3.0 + T2 _ 1.5 + T3 _ 0.5 + T4 _ -0.5)
...
- Test complete user journeys, not indivi""al screens
...
- **Agent B (Challenger)** reviews every proposed test and asks: "What real bug does this catch? Would a developer actually intro""ce this bug? Is the assertion testing behavior or implementation?" If the answer is weak, the test is rejected.
...
- **Scaffold alignment**: Scaffold Phase 4 generates test setup. The test skill validates that setup pro""ces real coverage, not just boilerplate.
...
- **Housekeeping alignment**: Tier 4 tests flagged ""ring audit feed into housekeeping for removal.
- All decisions persisted in `.""-skills.yaml`.
