---
name: housekeeping
description: Code housekeeping and consolidation. Use when the user wants to clean up redundant code, remove unused dependencies, deduplicate logic, consolidate styles/components/functions, and maximize code sharing in monorepos. Aggressive by default.
...
1. **Target project**: Ask the user for the project path or git URL.
   ...
   - If a local path: offer to create a git worktree in `/tmp/du-skills/<project-slug>-housekeeping` (recommended: changes can be reviewed before merging back)
...
1. Read `.du-skills.yaml` for housekeeping config (dedup_threshold, last_run).

2. **Explicitly prompt for scope** if not provided via arguments:
   ```
   Housekeeping scope not specified. What should I analyze?
   - deps: Dependencies only (unused, duplicates, outdated, heavy)
   - code: Code deduplication and dead code removal
   - styles: CSS/style consolidation, component deduplication
...
- Inline styles that should be extracted to stylesheets/modules
...
- Environment variables that should be in a config module
...
- Types/interfaces duplicated across packages (should be in `shared`)
...
- Constants (routes, status codes, error messages) duplicated
...
- 3 duplicate-purpose packages (consolidate to save 2 deps)
- 8 outdated dependencies (5 with security advisories)

### Code Deduplication
- 15 near-identical code blocks (consolidate to save ~450 lines)
- 8 genericization opportunities (reduce 24 components to 8)
...
### Monorepo Sharing
- 18 types duplicated across packages
...
- Consolidate X duplicate dependencies
- Remove ~X lines of duplicate code
...
From `.du-skills.yaml` `housekeeping.dedup_threshold`:
...
| **moderate** | Remove clearly unused deps, merge obviously duplicated code, flag (but don't auto-fix) borderline cases |
...
- **Agent B (Challenger)** independently verifies each target: is this code truly unused? Could it be loaded dynamically? Is this really a duplicate or does it have subtle differences?
