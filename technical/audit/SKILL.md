...
**Core principles**:
- A project scaffolded with `/scaffold` and audited with `/audit` should produce near-zero findings. Two consecutive `/audit` runs with fixes converge to zero Critical/High.
- Security and privacy findings are **never** downgraded to save time. A data exposure is Critical even if it's in a beta.
- Performance findings are evaluated against the project's stated targets (from docs or `.du-skills.yaml`). Missing targets default to: TTFB <3s, bundle <50MB, cold start <2s.
- Code consolidation is treated as a first-class dimension, not a subset of quality. Duplicated code is wasted budget.
...
2. If a git URL is provided: clone to `/tmp/du-skills/<project-slug>`.
3. If a local path is provided: offer a git worktree in `/tmp/du-skills/<project-slug>-audit`.
...
1. Read `.du-skills.yaml`. If absent, create it via auto-detection (see [stack-detection.md](../shared/stack-detection.md)).
...
**6-10 expert auditors** working in parallel. Each is independent, performs web searches for latest best practices and CVEs, and produces findings with file:line references and concrete fix code.
...
| A05 | Security Misconfiguration | Debug mode in production, default credentials, overly permissive CORS (wildcard origin with credentials), missing security headers (CSP, HSTS, X-Frame-Options, X-Content-Type-Options), exposed error stacks, open admin panels |
...
| GraphQL abuse | Unlimited query depth, introspection enabled in production, missing query cost analysis |
...
5. Check for exposed debug/admin endpoints in production config
...
| Unoptimized images (PNG when WebP/AVIF suffice, missing responsive srcset) | Medium | Check image imports, verify DU Shrink integration or build-time optimization |
...
| Heavy computation on JS thread (should be on Reanimated worklet or native module) | High | Look for complex calculations in render path or animation callbacks |
...
|-------------|-------------------|-------|
| **Memory (in-process)** | Computed values, parsed schemas, small lookup tables | Verify useMemo for expensive derivations, verify query deduplication (TanStack Query / SWR) |
...
| Check | Severity | Detection |
|-------|----------|-----------|
| Missing request deduplication (same data fetched multiple times) | Medium | Verify TanStack Query / SWR or equivalent deduplication layer |
...
The Consolidation Auditor enforces aggressive code reuse. Every line of duplicated code is a bug waiting to happen and budget wasted twice. The monorepo exists to share code. Use it.
...
| Same type/interface defined locally instead of imported from packages/shared | High | Grep for duplicate `type` and `interface` declarations across packages |
| Same Zod schema defined in multiple places | High | Grep for duplicate `z.object` definitions |
...
| Duplicate layout patterns (same header/footer/card structure reimplemented) | High | Compare layout structures across screens, flag structural duplication |
...
| Paid SaaS where self-hosted or cheaper alternative exists | High | Flag any paid API. Verify against approved vendor list: Expo Push (free), Brevo (email), Amazon SES (mass email >100k/month), Telnyx (SMS/MMS), Stripe/PayPal (payments), BitPay/Coinbase Commerce (crypto), DU Maps (maps, Google Maps as fallback). |
...
| Google Maps or Mapbox instead of DU Maps | High | DU Maps is preferred (free, self-hosted, API-compatible). Google Maps acceptable only if client contract requires it. Mapbox not approved. |
| Payment gateway not in approved list (Stripe/PayPal/BitPay) | High | Approved stack: Stripe (preferred, best DX/webhooks/Connect for marketplaces), PayPal (when client/market requires), BitPay (crypto only). Flag Square, Adyen, Braintree, Mollie, or custom integrations unless justified in .du-skills.yaml. |
...
| Any vendor diverging from approved list without written justification in .du-skills.yaml | High | Every divergence must have a decision record with date, reason, and who approved it. Unjustified divergences are flagged as High. |

**What the Consolidation Auditor must do:**
1. Map every type, hook, schema, utility, and component across all packages
2. Identify every instance of duplication (exact or structural)
3. For each duplication: propose where the shared version should live and how to refactor
...
7. **Audit every external dependency** against the approved vendor list and flag redundant or suboptimal choices
...
| Inconsistent naming (camelCase vs snake_case vs PascalCase mixing) | Low | Verify naming follows conventions from .du-skills.yaml |
...
| Console.log in production code | Low | Grep for `console.log` outside test files |
...
| Check | Severity | Detection |
|-------|----------|-----------|
| Zero test files | Critical | No *.test.ts, *.spec.ts, *_test.dart files |
...
| High | Significant: data exposure, broken user flow, major performance degradation, compliance violation | 1.0 | XSS, PII in logs, N+1 on main screen, missing RLS on user table, duplicated business logic |
| Medium | Moderate: non-critical performance, partial compliance, code maintainability | 0.5 | Missing cache layer, `any` type, hardcoded color, missing empty state, medium duplication |
...
2. Count files and LOC by language (exclude node_modules/vendor/dist/.git)
3. Identify tech stack and read `.du-skills.yaml` for architectural context
...
3. Produces findings with: file:line, severity, description, proof (code snippet), remediation (fix code), effort estimate
...
1. **Adjacent review**: Each auditor reviews findings from overlapping domains:
   ...
   - Performance Auditor reviews Consolidation findings (duplication's performance cost)
...
4. **Deduplication**: Merge duplicate findings flagged by multiple auditors. Keep the most detailed version with the highest severity.
...
6. Update `.du-skills.yaml` with audit timestamp, overall score, per-dimension scores, finding counts
...
1. Default: `./reports/audit-<YYYY-MM-DD>/` in the target project (or `~/Documents/du-audits/<project>/` if user prefers not to write into the project).
2. Files:
   ...
   - `consolidation.md` : Code reuse findings (duplication, sharing, atomic components, genericity)
...
- All findings persisted in `.du-skills.yaml`
