...
1. Read `.du-skills.yaml` for review config (require_tests, coverage_check, max_pr_size).
...
3. Read the test skill's coverage expectations from `.du-skills.yaml` to know what test standards to enforce.
...
- **Agent A (Executor)** performs the initial review, producing findings and severity ratings
